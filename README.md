...
## Project Origin
This project is forked from [supercharged-rails-api-template](https://github.com/windurisky/supercharged-rails-api-template), an open-source project by [me](https://github.com/windurisky) that simplifies starting a new backend Rails service.
It is designed to be lightweight, scalable, and developer-friendly.

## GitHub Codespace Compatibility
This project is fully compatible with GitHub Codespace (cloud development server).
You can try it directly by starting up a Codespace, once the Codespace is ready, run these below (on separate terminal session):
...
2. Start sidekiq worker
   ...
   bundle exec sidekiq
   ```
3. Make the port public (there should be a button to do that once rails server is up)
4. Open the swagger UI `https://*.github.dev/api-docs/index.html` on your local machine
